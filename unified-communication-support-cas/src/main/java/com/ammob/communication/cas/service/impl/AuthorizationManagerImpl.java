...
import com.ammob.communication.cas.service.AuthorizationService;
import com.ammob.communication.core.util.StringUtil;

@Service("authorizationManager")
@WebService(serviceName = "AuthorizationService", 
	endpointInterface = "com.ammob.communication.cas.service.AuthorizationService")
public class AuthorizationManagerImpl implements AuthorizationService {
	...
	public Response getGrantTicket(String username, String password, boolean rememberMe, UriInfo uriInfo) {
        try {
        	if(StringUtil.hasText(username) && StringUtil.hasText(username) && uriInfo != null) {
	            String tgt = this.centralAuthenticationService.createTicketGrantingTicket(new VidyoCredentials(username, password, "http://v.seekoom.com"));
