...
<div class="span7">
    ...
    <form:form commandName="userForm" method="post" action="userform" id="userForm" autocomplete="off"
        ...
        <c:if test="${empty userForm.version}">
    ...
    <c:if test="${cookieLogin != 'true'}">
        ...
        <spring:bind path="userForm.nickname">
        <fieldset class="control-group${(not empty status.errorMessage) ? ' error' : ''}">
        </spring:bind>
            <appfuse:label styleClass="control-label" key="userForm.nickname"/>
            <div class="controls">
                <form:input path="nickname" id="nickname" maxlength="50"/>
...
<c:choose>
        ...
        <fieldset class="control-group">
            ...
            <div class="controls">
                <select id="userRoles" name="userRoles" multiple="true">
                    ...
                    <option value="${role.value}" ${fn:contains(userForm.roles, role.label) ? 'selected' : ''}>${role.label}</option>
                    </c:forEach>
                </select>
            </div>
        </fieldset>
    ...
    <c:when test="${not empty userForm.username}">
        <fieldset class="control-group">
            <label class="control-label"><fmt:message key="userForm.roles"/>:</label>
            <div class="controls readonly">
                <c:forEach var="role" items="${userForm.roleList}" varStatus="status">
...
<script type="text/javascript">
    function passwordChanged(passwordField) {
        if (passwordField.id == "password") {
            ...
            var origPassword = "${userForm.confirmPassword}";
        }
